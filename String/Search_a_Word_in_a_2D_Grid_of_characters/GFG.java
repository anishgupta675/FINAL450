...
class Solution
{
    ...
    public boolean search(char[][] grid,int i,int j,char[] word,int rows,int cols, int word_len){
        ...
        for(int k = 0 ; k < 8 ; k++){
            int di = i, dj = j;
            int ind = 0;
            /*check while characters keep matching if not matched proceed in next direction*/
            while(ind < word_len && isValid(di, dj, rows, cols) && word[ind] == grid[di][dj]){
                 di += directions[k][0];
                 dj += directions[k][1];
                 ind++;
            }
            if(ind == word_len) return true;
        }
